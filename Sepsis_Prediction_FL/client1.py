...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        coef = self.model.coef_.flatten()          # Shape: (n_features,)
        intercept = self.model.intercept_[0]       # Scalar

        # Logging
        print(f"[Client {client_id}] Trained Coefficients: {coef}")
        print(f"[Client {client_id}] Trained Intercept: {intercept}")
        ...
        return (
            ...
            "coef": float(self.model.coef_.flatten()[0])  # or str(...) if you need full list
            }
